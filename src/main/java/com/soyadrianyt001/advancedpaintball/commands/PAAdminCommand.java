...
public class PAAdminCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
        if (!s.hasPermission("advancedpaintball.admin")) {
            s.sendMessage(Msg.err("Sin permiso."));
            return true;
        }
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "wand" -> {
                if (args.length < 2) {
                    p.sendMessage(Msg.err("Uso: /pa-admin wand <arena>"));
                    return true;
                }
                if (!plugin.getArenaManager().exists(args[1])) {
                    p.sendMessage(Msg.err("Arena no existe."));
                    return true;
                }
            ...
            case "crear", "create" -> {
                if (args.length < 2) {
                    p.sendMessage(Msg.err("Uso: /pa-admin crear <nombre>"));
                    return true;
                }
                if (plugin.getArenaManager().exists(args[1])) {
                    p.sendMessage(Msg.err("Esa arena ya existe."));
                    return true;
                }
                Arena a = plugin.getArenaManager().create(args[1]);
                plugin.getArenaManager().save(a);
                p.sendMessage(Msg.ok("Arena &f" + args[1]
                    + " &acreada. Usa &f/pa-admin panel &apara configurarla."));
            }

            case "eliminar", "delete" -> {
                if (args.length < 2) {
                    p.sendMessage(Msg.err("Uso: /pa-admin eliminar <nombre>"));
                    return true;
                }
                if (!plugin.getArenaManager().exists(args[1])) {
                    p.sendMessage(Msg.err("Arena no existe."));
            ...
            case "setlobby" -> {
                if (args.length < 2) {
                    p.sendMessage(Msg.err("Uso: /pa-admin setlobby <arena>"));
                    return true;
                }
            ...
            case "setspawn" -> {
                if (args.length < 3) {
                    p.sendMessage(Msg.err("Uso: /pa-admin setspawn <arena> <rojo|rosa|verde|amarillo>"));
                    return true;
                }
                Arena a = plugin.getArenaManager().get(args[1]);
                if (a == null) {
                ...
                switch (args[2].toLowerCase()) {
                    ...
                    default         -> p.sendMessage(Msg.err("Equipos: rojo, rosa, verde, amarillo"));
                }
                plugin.getArenaManager().save(a);
            }
            ...
            case "info" -> {
                if (args.length < 2) {
                    p.sendMessage(Msg.err("Uso: /pa-admin info <arena>"));
                    return true;
                }
                Arena a = plugin.getArenaManager().get(args[1]);
                if (a == null) {
                ...
                p.sendMessage(Msg.c("  &7Visible /pa:  " + (a.isVisibleInMenu() ? "&a✔" : "&c✗")));
            ...
            case "npc" -> {
                if (args.length < 2) {
                    p.sendMessage(Msg.err("Uso: /pa-admin npc <crear|eliminar> [arena]"));
                    return true;
                }
                if (args[1].equalsIgnoreCase("crear")) {
                    if (args.length < 3) {
                        p.sendMessage(Msg.err("Uso: /pa-admin npc crear <arena>"));
                        return true;
                    }
            ...
            case "holo" -> {
                if (args.length < 2) {
                    p.sendMessage(Msg.err("Uso: /pa-admin holo <settop|removetop|update>"));
                    return true;
                }
                switch (args[1].toLowerCase()) {
                    case "settop" -> {
                        plugin.getHologramManager().setTopLocation(p.getLocation());
                        p.sendMessage(Msg.ok("Holograma Top10 creado en tu posición!"));
                    }
                    case "removetop" -> {
                        plugin.getHologramManager().removeTop();
                        p.sendMessage(Msg.ok("Holograma eliminado."));
                    }
                    case "update" -> {
                        plugin.getHologramManager().updateTop();
                        p.sendMessage(Msg.ok("Holograma actualizado."));
                    }
                    default -> p.sendMessage(Msg.err("Opciones: settop, removetop, update"));
                }
            }
            ...
            case "sign" -> {
                if (args.length < 3) {
                    p.sendMessage(Msg.err("Uso: /pa-admin sign <crear|eliminar> <arena>"));
                    return true;
                }
                var targetBlock = p.getTargetBlockExact(5);
                if (targetBlock == null || !(targetBlock.getState() instanceof Sign)) {
                    p.sendMessage(Msg.err("Mira un letrero (sign) a menos de 5 bloques."));
                    return true;
                }
                if (args[1].equalsIgnoreCase("crear")) {
                    if (!plugin.getArenaManager().exists(args[2])) {
                        p.sendMessage(Msg.err("Arena no existe."));
                        return true;
                    }
                    plugin.getSignManager().addSign(targetBlock.getLocation(), args[2]);
                    p.sendMessage(Msg.ok("Sign registrado para arena &f" + args[2]));
                } else if (args[1].equalsIgnoreCase("eliminar")) {
                    plugin.getSignManager().removeSign(targetBlock.getLocation());
                    p.sendMessage(Msg.ok("Sign eliminado."));
                }
            }

            case "reload" -> {
                plugin.reloadConfig();
                p.sendMessage(Msg.ok("Config recargada."));
            }
    ...
    private void sendHelp(Player p) {
        ...
        p.sendMessage(Msg.c("  &f/pa-admin eliminar <nombre> &7— Eliminar arena"));
        ...
        p.sendMessage(Msg.c("  &f/pa-admin holo settop &7— Poner holo Top10"));
        p.sendMessage(Msg.c("  &f/pa-admin holo removetop &7— Quitar holo"));
        p.sendMessage(Msg.c("  &f/pa-admin holo update &7— Actualizar holo"));
        p.sendMessage(Msg.c("  &f/pa-admin sign crear <arena> &7— Crear sign"));
        p.sendMessage(Msg.c("  &f/pa-admin sign eliminar <arena> &7— Quitar sign"));
    ...
    public List<String> onTabComplete(CommandSender s, Command c,
        ...
        if (args.length == 1) {
            out.addAll(Arrays.asList(
                "panel", "wand", "crear", "eliminar", "setlobby",
                "setspawn", "setlobby-principal", "info", "npc",
                "holo", "sign", "reload"
            ));
        } else if (args.length == 2) {
            switch (args[0].toLowerCase()) {
                case "wand", "eliminar", "setlobby", "info" ->
                    plugin.getArenaManager().all().forEach(a -> out.add(a.getName()));
                case "npc"  -> out.addAll(Arrays.asList("crear", "eliminar"));
                case "holo" -> out.addAll(Arrays.asList("settop", "removetop", "update"));
                case "sign" -> out.addAll(Arrays.asList("crear", "eliminar"));
            }
        } else if (args.length == 3) {
            switch (args[0].toLowerCase()) {
                case "setspawn" ->
                    out.addAll(Arrays.asList("rojo", "rosa", "verde", "amarillo"));
                case "npc", "sign" ->
                    plugin.getArenaManager().all().forEach(a -> out.add(a.getName()));
            }
        }
